        ...
        <div class="max-w-4xl w-full space-y-8 bg-white shadow-2xl rounded-lg p-10">
            ...
            <form action="{{ route('alumnos.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- Botón -->
                    <div class="mt-8 md:col-span-2 flex justify-between gap-4">
                        <!-- Volver a grupos (Izquierda) -->
                        <a href="{{ route('grupos.show', $grupoId) }}" 
                            class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#285C4D] hover:bg-[#13322B] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#621132] transition duration-300 ease-in-out transform hover:scale-105">
                            Volver
                        </a>
                    
                        <!-- Registrar Alumno (Derecha) -->
                        <button type="submit"
                            class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#9D2449] hover:bg-[#621132] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#621132] transition duration-300 ease-in-out transform hover:scale-105">
                            Registrar Alumno
                        </button>
                    </div>
